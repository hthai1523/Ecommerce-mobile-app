  ...
  addProduct: (product: ProductWithSizeColor) =>
    set((state) => {
      ...
      const hasProduct = state.products.find(
        (p) => p.productID === product.productID && p.selectedSize === product.selectedSize && p.selectedColor === product.selectedColor
      );
      if (hasProduct) {
        return {
    ...
    set((state) => {
      ...
      return {
        products: state.products
          .map((p) => {
            if (p.productID === product.productID && p.selectedSize === product.selectedSize && p.selectedColor === product.selectedColor) {
              return {
                ...p,
