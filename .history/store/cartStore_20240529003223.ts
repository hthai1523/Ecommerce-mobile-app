  ...
  addProduct: (product: ProductDetail) =>
    set((state) => {
      ...
      const hasProduct = state.products.find(
        (p) => p.productID === product.productID && p.sizes.filter(p => p === product.sizes[0]) && p.colors.filter(p => p === product.colors[0])
      );
      if (hasProduct) {
        return {
          products: state.products.map((p) =>
            p.productID === product.productID && p.sizes.filter(p => p === product.sizes[0]) && p.colors.filter(p => p === product.colors[0])
