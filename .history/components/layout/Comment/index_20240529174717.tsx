...
const Comment = ({ productID }: { productID: number }) => {
  ...
  const postComment = async () => {
    ...
    if (user) {
      try {
        const response = await axios.post(`http://192.168.2.10:8070/api/v1/Comments/insert`, {
          idProduct: productID,
          name: username,
        ...
        setInput(""); // Clear input after successful posting
      } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => <CommentSection key={item.id} {...item} />}
      />
    </View>
  );
};
