...
const Comment = ({ product }: { product: Product }) => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const productDoc = await getDoc(doc(FIREBASE_DB, "comments", product.id));
        if (productDoc.exists()) {
          const data = productDoc.data().comments;
          setComments(data);
        } else {
          setComments([]);
        }
  ...
  const postComment = async () => {
    ...
    if (user) {
      try {
        ...
        const productDocRef = doc(FIREBASE_DB, "comments", product.id);
        const productDoc = await getDoc(productDocRef);
        let updatedComments = [];

        if (productDoc.exists()) {
          updatedComments = [...productDoc.data().comments, newComment];
        } else {
          updatedComments = [newComment];
        }

        await setDoc(productDocRef, { comments: updatedComments }, { merge: true });
