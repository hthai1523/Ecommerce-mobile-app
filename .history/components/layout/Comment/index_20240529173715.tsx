...
const Comment = ({ productID }: { productID: number }) => {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(`http://192.168.2.10:8070/api/v1/Comments/product/${productID}`);
  ...
  const postComment = async () => {
    ...
    if(user) {
      try {
        await axios.post(`http://192.168.2.10:8070/api/v1/Comments/insert`, {
          idProduct: productID,
          name: username, 
      ...
      Alert.alert(
        "Login Required",
        "You need to log in to add to cart. Would you like to log in?",
        [
          {
            text: "Cancel",
            style: "cancel",
