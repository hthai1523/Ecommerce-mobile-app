      ...
      >
        {
          categories.map((category, index) => (
            <TouchableOpacity onPress={() => selectCategory(index)} className={activeIndex === index ? 'bg-sky-300' : 'bg-red-400'} key={index}>
              <Text className={activeIndex === index ? '' : 'bg-red-400'}>{category}</Text>
            </TouchableOpacity>
          ))
        }
      </ScrollView>
