  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <ScrollView
        ...
        className="absolute h-10 l-0 r-0 top-16"
      >
        {
          categories.map((category, index) => (
            <TouchableOpacity onPress={() => selectCategory(index)} className={activeIndex === index ? 'bg-black' : 'bg-red-400'} key={index}>
              <Text className=className={activeIndex === index ? 'bg-black' : 'bg-red-400'}>{category}</Text>
            </TouchableOpacity>
          ))
        }
      </ScrollView>
