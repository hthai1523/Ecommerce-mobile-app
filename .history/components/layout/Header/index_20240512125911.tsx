      ...
      <View className="bg-white h-[53px] flex flex-row items-center justify-between border-b border-b-[#EEE]">
        {isBack && (
          <TouchableOpacity onPress={() => router.back()} className="">
            <Feather name="chevron-left" size={24} color={'#87CEFA'} />
          </TouchableOpacity>
        )}
        <View className="flex-row min-w-[239] items-center px-3 ml-3 rounded-md bg-[#E0E0E0]">
          <TouchableOpacity>
            <Feather name="search" size={20} color={"#ADADAD"} />
          </TouchableOpacity>
          <Link href={"/(modals)/search"} asChild>
            <Pressable className="px-2 py-2 flex-1">
              <Text className="text-[#ADADAD]"> Anything</Text>
            </Pressable>
          </Link>
        </View>
