...
const Page = () => {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      try { 
        setIsLoading(true);
        const data = await api.get(`https://dummyjson.com/products/${id}`);
  ...
  const priceReal = product && (product.price - (product.price * (product.discountPercentage / 100)))
