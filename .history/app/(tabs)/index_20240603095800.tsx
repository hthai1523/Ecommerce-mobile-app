...
const Page = () => {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      try {
        ...
        const data = await getProductArrivals()
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: 'white'}}>
      ...
      />
      <ScrollView showsVerticalScrollIndicator={false} className="mt-[53] bg-white ">
        <View className="mt-4">
          <Slider />
        </View>
        <Separate color="#EEE" />

        <ShoppingList data={listProductRecommended} title="Recommended" isLoading={isLoading} isNavigation={false} />

        <Separate color="#EEE" />

        <ShoppingList data={listProduct} title="Official Store" isLoading={isLoading} isNavigation={true} />
