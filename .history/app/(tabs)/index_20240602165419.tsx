  ...
  useEffect(() => {
    const fetchApi = async () => {
      try {
        ...
        const data = await axios.get('https://dummyjson.com/products?sortBy=createdAt&order=asc')

        setListProductArrivals(data.data.product);
        setListProductRecommended(sortedProduct)
        setIsLoading(false);
      } catch (error) {
        console.error(error);
        setIsLoading(true);
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="mt-[53] ">
        ...
        <View className="ml-5">
          <Text className="text-lg font-extrabold">Recommended</Text>
          {isLoading ? (
            <ActivityIndicator
              size={"large"}
