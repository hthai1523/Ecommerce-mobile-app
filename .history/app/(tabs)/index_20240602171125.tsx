...
const Page = () => {
  const [listProductRecommened, setListProductRecommended] = useState<Product[]>([]);
  ...
  useEffect(() => {
    const fetchApi = async () => {
      try {
        ...
        setListProductRecommended(data);
        setListProduct(data2)
        setIsLoading(false);
      } catch (error) {
        console.error(error);
        setIsLoading(true);
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="mt-[53] ">
        ...
        <View className="ml-5">
          ...
          {isLoading ? (
            ...
            >
              {listProductRecommened.map((item) => {
                return (
