...
const ShoppingCart = () => {
  ...
  const totalPriceCart = () => {
    return products.reduce((acc, item) => acc + (item.price - (item.price * (item.discountPercentage / 100))) * item.quantity, 0);
  };
  const totalPriceNoDiscountCart = () => {
    return products.reduce((acc, item) => acc + item.price * item.quantity - (item.price - (item.price * (item.discountPercentage / 100))) * item.quantity, 0);
  };
          ...
          <View className="py-3  px-2">
            ...
            <Text className="text-sm font-extralight">
              ...
              <Text className="text-sky-400 font-normal text-base">
                {formatCurrency(totalPriceNoDiscountCart())}
              </Text>
            </Text>
          </View>
          <TouchableOpacity
