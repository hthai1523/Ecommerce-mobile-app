  ...
  return (
    <View className="bg-white h-full">
      <View className="flex-row items-center justify-between p-4 border-b border-b-[#ADADAD]">
        ...
        <Text className="text-base font-bold">{formatCurrency(totalPriceCart())}</Text>
      </View>

      <View className="w-full pt-6  space-y-4">
        <View className="flex-row justify-between items-center px-4">
          <Text className="text-lg font-normal">Purchase Items</Text>
          <Text className="text-base font-semibold">Total Items: {products.length}</Text>
        </View>
        {/* cart section */}
        <View className="w-full h-80 px-1">
          <FlatList
            data={products}
            renderItem={(item) => (
