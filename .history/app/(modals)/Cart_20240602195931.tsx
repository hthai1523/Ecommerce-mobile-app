...
const ShoppingCart = () => {
  ...
  const totalPriceCart = () => {
    return products.reduce((acc, item) => acc + (item.price * (item.discountPercentage / 100)) * item.quantity, 0);
  };
  const totalPriceNoDiscountCart = () => {
    return products.reduce((acc, item) => acc + item.price * item.quantity - (item.price * (item.discountPercentage / 100)) * item.quantity, 0);
  };
